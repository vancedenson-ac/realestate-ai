...
- `GET /v1/transactions/{transaction_id}/documents` — Returns document list; each item includes optional `view_url` (presigned GET for latest version) when at least one version exists.

#### Get document

- `GET /v1/documents/{document_id}` — Returns document overview; includes `view_url` when at least one version exists.
...
- `GET /v1/properties/{property_id}/images` — List images with variants; each item includes optional `view_url` (presigned GET) when upload is complete (file_size > 0, checksum not "pending")
